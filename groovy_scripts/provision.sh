...
# A simple example script that publishes a number of scripts to the Nexus Repository Manager and executes them.
...
# add context if not using the root context
...
function addAndRunScript {
  name=$1
  file=$2
  # using grape config that points to local Maven repo and Central Repository 
